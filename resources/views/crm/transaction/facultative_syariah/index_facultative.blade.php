                  ...
                  <table id="felookupTable2" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @foreach (@$insuredlist as $insureddata)
                            <tr>
                              <td ><a href="{{  url('transaction-data/'.$insureddata->slip_type.'/detailform', $insureddata->id) }}">{{@$insureddata->number}}</a></td>
                              <td> @if($insureddata->prefix_id = 1)
                                   PT - 
                                   @else
                                   CV - 
                                   @endif
...
@section('scripts')
@include('crm.transaction.facultative_syariah.js.index_facultative_js')
@endsection