<script type="text/javascript">$(document).ready(function() 
    { 
        ...
        var currency_exc = $('#fecurrency').val();
        if(currency_exc){
            $.ajax({
                type:"GET",
    ...
    $('#feshareto').change(function(){
        ...
        if(sumpct != '' && typetsi == '1'){
                ...
                if(dpamount != null){
                    ...
                    $('#slipdpamount').val(convert(sum));
                    $('#slipdpamount2').val(convert(sum));
                }
                ...
                if(ecamount != null){
                    ...
                    $('#slipamountec').val(convert(sum));
                    $('#slipamountec2').val(convert(sum));
                        ...
                        if(response.success == false){
                            ...
                            $('#tcid'+insurednoloc+' > tbody:last-child').prepend('<tr id="riskdetailsid'+response.id+'">'+
                               ...
                               console.log("coba dulu ini " + $("#slipcedingbroker option[value='"+response.idceding+"']").text())
                                if(test){
                                    console.log('this works on slip insert')
                                ...
                                if(response.sumceding){
                                        ...
                                        for(var i = 0; i < total_amount_ceding.length; i++){
                                            ...
                                            '<td >'+name+'</td>'+
                                            '<td >'+new_percent+'%</td>'+
                                            '<td >'+floatToString(amount_ceding)+'</td>'+
                                            '</tr>');
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                // $('#feshareto').val(real_sumtotalnre);
                    ...
                    console.log('jumlah ceding' + response.jumlahceding)

                    if(response.jumlahceding > 0){
...
<script type='text/javascript'>
    $('#addinsuredsave-btn').click(function(e){
       //alert('masuk');
        ...
        console.log('conv_fesshare' + conv_fesshare)
        var real_fesshare = parseFloat(conv_fesshare);
        console.log('real_fesshare' + real_fesshare)
        var conv_fessharefrom = fessharefrom.replace(/,/g, "");
        console.log('conv_fessharefrom ' + conv_fessharefrom)
        var real_fessharefrom = parseFloat(conv_fessharefrom);
        console.log('real_fessharefrom' + real_fessharefrom)
        var conv_fesshareto = fesshareto.replace(/,/g, "");
        console.log('conv_fesshareto' + conv_fesshareto)
        var real_fesshareto = parseFloat(conv_fesshareto);
        console.log('real_fesshareto' + real_fesshareto)
        ...
        if(febusiness_id == '' || febusiness_id == null){
            swal('Error!','Field Business Type first','warning')
        }
        if(fessuggestinsured == '' || fesshareto == ''){
            ...
            $.ajax({
            url:"{{ url('transaction-data/fe/storeheader') }}",
            type:"POST",
            data:{
                fesnumber:fesnumber,
            ...
            success:function(response)
            {
                ...
                window.location.href = "{{ url('transaction-data/fe/detailform/')}}/"+btoa(response.id);    
...
<script type="text/javascript">
    function getTsi(){
        ...
        if(valtsi != null){
            ...
            $('#amountlocation').val(floatToString(parseFloat(sum_amount).toFixed(2)));
        }
        else
        {
    ...
    // function refreshLocationDetail(id){
    //     var token = $('input[name=_token]').val();
    //     var sliptype = 'fe';
    //     $('#tcid'+id+'').empty();
    //     $.ajax({
    //                 url:"{{ route('locationlist.refresh') }}",
    //                 type:"POST",
    //                 data:{
    //                     id:id,
    //                     sliptype:sliptype,
    ...
    //                     $('#tcid'+id+' > tbody:last-child').prepend('<tr id="riskdetailsid'+response.id+'">'+
    //                         '<td>'+response.cedingbroker+'</td>'+
    //                         '<td>'+response.interest_name+'</td>'+
    //                         '<td>'+cndn+'</td>'+
    //                         '<td>'+certno+'</td>'+
