...
class FeLookupLocationController extends Controller
{
    ...
    public function index(Request $request)
    {
         ...
         $searchcity = @$request->input('searchcity');
         $searcheqzone = @$request->input('searcheqzone');
         $searchfloodzone = @$request->input('searchfloodzone');
         $searchinsured = @$request->input('searchinsured');

         if(!empty($searchcode) || !empty($searchcountry) || !empty($searchprovince) || !empty($searchcity)  || !empty($searcheqzone)  || !empty($searchfloodzone)  || !empty($searchinsured))
         {
                $query = FeLookupLocation::query();

                if (!empty($searchcode)) {
                    $query = $query->where('loc_code', $searchcode);
                }

                if (!empty($searchcountry)) {
                    $query = $query->where('country_id', $searchcountry);
                }

                if (!empty($searchprovince)) {
                    $query = $query->where('province_id', $searchprovince);
                }

                if (!empty($searchcity)) {
                    $query = $query->where('city_id', $searchcity);
                }

                if (!empty($searcheqzone)) {
                    $query = $query->where('eq_zone', $searcheqzone);
                }

                if (!empty($searchfloodzone)) {
                    $query = $query->where('flood_zone', $searchfloodzone);
                }

                if (!empty($searchinsured)) {
                    $query = $query->where('insured','LIKE' ,'%'.$searchinsured.'%');
                }

                
                $felookuplocation=$query->orderby('id','desc')->get();

                $felookuplocationsearch=FeLookupLocation::select('loc_code','address','postal_code')->orderBy('created_at','desc')->get();
                ...
                return view('crm.master.felookuplocation', compact('user','felookuplocationsearch','code_felookuplocation','earthquakezone','floodzone','felookuplocation','costumer','route_active','felookuplocation_ids','country','city','state'))->with('i', ($request->input('page', 1) - 1) * 10);
